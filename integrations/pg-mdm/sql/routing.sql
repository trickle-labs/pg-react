...
BEGIN
    ...
    RETURN QUERY EXECUTE format($query$
        WITH source_rows AS (
            ...
            SELECT c.case_key, c.review_id, c.reason_code, c.assigned_queue,
                   ...
                   jsonb_build_object(
                       ...
                       'queue', route ->> 'queue',
                       'priority', (route ->> 'priority')::integer,
                       'action', COALESCE(route ->> 'action', 'ASSIGN_QUEUE')) AS candidate,
                   (route ->> 'priority')::integer AS candidate_priority,
        ...
        SELECT c.case_key, c.review_id, $2, c.reason_code, c.assigned_queue,
               CASE WHEN d.decision = 'WINNER' THEN d.best_queue ELSE NULL END,
               d.decision,
               ...
               jsonb_build_object(
                   ...
                   'decision', d.decision,
                   'selected_queue', CASE WHEN d.decision = 'WINNER' THEN d.best_queue ELSE NULL END,
