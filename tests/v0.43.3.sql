...
BEGIN
    ...
    IF declaration IS DISTINCT FROM expected THEN
        RAISE EXCEPTION '0.43.3 canonical rule output mismatch: %', declaration;
    END IF;
    ...
    IF validation ->> 'state' <> 'ready'
       OR validation -> 'evidence' -> 'normalized_declaration' IS DISTINCT FROM
        ...
        RAISE EXCEPTION '0.43.3 quoted validation output mismatch: %', validation;
    END IF;
    ...
    IF first_result ->> 'state' <> 'deployed'
       ...
       OR first_result -> 'summary' ->> 'delegated_id' IS NULL THEN
        RAISE EXCEPTION '0.43.3 allow_create=false creation mismatch: %', first_result;
    END IF;
    declaration := pgreact.rule(
    ...
    IF second_result ->> 'state' <> 'deployed'
       OR second_result -> 'summary' ->> 'action' <> 'REPLACE' THEN
        RAISE EXCEPTION '0.43.3 guarded replacement mismatch: %', second_result;
    END IF;
    BEGIN
        PERFORM pgreact.deploy(
            ...
            jsonb_build_object('allow_create', false,
                'expected_current_digest', first_result -> 'summary' ->> 'proposed_declaration_digest'));
        RAISE EXCEPTION '0.43.3 accepted a stale expected_current_digest';
    ...
    IF ordered IS DISTINCT FROM expected_order THEN
        RAISE EXCEPTION '0.43.3 package order mismatch: %', ordered;
    END IF;
    ...
    IF pgreact_internal.m54_package_has_cycle(package -> 'spec') IS DISTINCT FROM true THEN
        RAISE EXCEPTION '0.43.3 package cycle detection mismatch';
    END IF;
    ...
    IF validation ->> 'state' <> 'attention'
       OR NOT EXISTS (SELECT 1 FROM jsonb_array_elements(validation -> 'findings') item
        ...
        RAISE EXCEPTION '0.43.3 transitive RLS validation mismatch: %', validation;
    END IF;
END
$v0432$;

SELECT 'v0.43.3 SQL corpus passed' AS result;

DO $v0433$
DECLARE
    observed jsonb;
    expected jsonb := jsonb_build_object(
        'extension_version', '0.43.3',
        'sweep_return_type', 'bigint',
        'agenda_timestamp_type', 'timestamp with time zone',
        'agenda_timestamp_trigger', 'pgreact_agenda_state_timestamp',
        'work_has_retry_wait', true,
        'work_has_advisory_comment', true,
        'retry_uses_bounded_division', true);
BEGIN
    SELECT jsonb_build_object(
        'extension_version', (
            SELECT extversion FROM pg_extension WHERE extname = 'pg_react'),
        'sweep_return_type', pg_get_function_result(
            'pgreact.sweep_expired_leases(text)'::regprocedure),
        'agenda_timestamp_type', (
            SELECT format_type(atttypid, atttypmod)
            FROM pg_catalog.pg_attribute
            WHERE attrelid = 'pgreact_internal.agenda'::regclass
        ...
        'agenda_timestamp_trigger', (
            ...
            WHERE event_object_schema = 'pgreact_internal'
              ...
              AND trigger_name = 'pgreact_agenda_state_timestamp'),
        'work_has_retry_wait', position(
            'RETRY_WAIT' IN pg_get_viewdef('pgreact.work'::regclass, true)) > 0,
        'work_has_advisory_comment', (
            SELECT description = 'Advisory claimability; a concurrent transaction may claim after this read.'
            FROM pg_catalog.pg_description
            WHERE objoid = 'pgreact.work'::regclass
              AND objsubid = 6),
        'retry_uses_bounded_division', position(
            'retry_max_seconds::numeric /' IN pg_get_functiondef(
                'pgreact_internal.execute_claimed_episode_m7(bigint,text,uuid)'::regprocedure)) > 0)
    INTO observed;
    IF observed IS DISTINCT FROM expected THEN
        RAISE EXCEPTION '0.43.3 runtime contract mismatch: expected %, observed %',
            expected, observed;
    END IF;
END
$v0433$;

SELECT 'v0.43.3 runtime contract passed' AS result;
