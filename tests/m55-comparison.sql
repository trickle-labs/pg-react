...
BEGIN
    ...
    target_checksum := pgreact_internal.m55_target_provenance(
        'rule', 'm34-order-rule', NULL,
        pgreact_internal.m28_normalize(proposed_declaration));
    ...
    IF comparison ->> 'contract_version' <> '55'
       OR comparison -> 'evidence' -> 'provenance' ->> 'scope' <> 'target'
       OR comparison -> 'evidence' -> 'provenance' ->> 'before' IS NULL
       ...
       OR comparison -> 'evidence' ->> 'authoritative_checksum_before' <> target_checksum
       OR comparison -> 'evidence' ->> 'authoritative_checksum_after' <> target_checksum
