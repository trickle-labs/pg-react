...
profile=${1:-fast}
image=${2:-pg-react:v0.19.0}
case "$profile" in fast|complete) ;; *) echo 'usage: tests/m22.sh fast|complete [IMAGE]' >&2; exit 2 ;; esac

project=${COMPOSE_PROJECT_NAME:-pgreact-m22-${GITHUB_RUN_ID:-$$}}
upgrade_project=${project}-upgrade
test_log_dir=$(mktemp -d)
artifact_dir=${M22_ARTIFACT_DIR:-}
...
wait_for_version "$project" 0.19.0
docker compose cp tests/m22.sql postgres:/tmp/m22.sql >/dev/null
run_test 'M22 bounded provenance public gate' \
  docker compose exec -T postgres psql -XAtq -U postgres -d postgres \
  -v ON_ERROR_STOP=1 -f /tmp/m22.sql

if [[ $profile = complete ]]; then
  export COMPOSE_PROJECT_NAME=$upgrade_project
  export PG_REACT_INIT_VERSION=0.18.0
  docker compose up -d --no-build >/dev/null 2>&1
  wait_for_version "$upgrade_project" 0.18.0
  docker compose cp tests/m22-upgrade-before.sql postgres:/tmp/m22-upgrade-before.sql >/dev/null
  docker compose cp tests/m22-upgrade-after.sql postgres:/tmp/m22-upgrade-after.sql >/dev/null
  run_test 'M22 populated direct-upgrade setup' \
    docker compose exec -T postgres psql -XAtq -U postgres -d postgres \
    -v ON_ERROR_STOP=1 -f /tmp/m22-upgrade-before.sql
  run_test 'M22 direct 0.18.0 to 0.19.0 upgrade' \
    docker compose exec -T postgres psql -XAtq -U postgres -d postgres \
    -v ON_ERROR_STOP=1 -c "ALTER EXTENSION pg_react UPDATE TO '0.19.0';"
  run_test 'M22 populated upgrade provenance preserved' \
    docker compose exec -T postgres psql -XAtq -U postgres -d postgres \
    -v ON_ERROR_STOP=1 -f /tmp/m22-upgrade-after.sql
fi

echo "M22 $profile evidence gate passed for $image (linux/amd64)"
if [[ -n $artifact_dir ]]; then
  mkdir -p -- "$artifact_dir"
  cp -- "$test_log_dir"/*.log tests/fixtures/m22/release-state.json "$artifact_dir"/
fi
