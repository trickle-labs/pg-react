...
BEGIN
    ...
    ELSIF (SELECT extversion FROM pg_catalog.pg_extension WHERE extname = 'pg_react') IN ('0.11.0', '0.12.0') THEN
        expected := ARRAY[
            'attempts', 'author_deadline_rule', 'author_rule', 'batch_status',
            'claim', 'claim_batch', 'configure_roles', 'deadline_history',
        ...
        IF (SELECT extversion FROM pg_catalog.pg_extension WHERE extname = 'pg_react') = '0.12.0' THEN
...
BEGIN
    ...
    IF actual IS DISTINCT FROM jsonb_build_object(
        'contract_version', CASE
            WHEN (SELECT extversion FROM pg_catalog.pg_extension WHERE extname = 'pg_react') IN ('0.9.0', '0.10.0', '0.11.0', '0.12.0')
            THEN 2 ELSE 1 END,
