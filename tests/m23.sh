...
if [[ ${M23_SKIP_REPO_AUDIT:-0} != 1 ]]; then
...
if [[ $profile = complete ]]; then
  run_test 'M22 inherited complete gate' env M22_SKIP_REPO_AUDIT="${M22_SKIP_REPO_AUDIT:-0}" \
    COMPOSE_PROJECT_NAME="${project}-inherited" tests/m22.sh complete "$image"
