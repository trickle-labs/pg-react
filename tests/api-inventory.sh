...
jq -e '
  .schema_version == 1 and .extension_version == "0.46.0" and
...
cmp docs/api-inventory.json docs/m55-api-inventory.json
...
if [[ -z "$image" ]] || ! command -v docker >/dev/null 2>&1 || ! docker image inspect "$image" >/dev/null 2>&1; then
  echo 'v0.46.0 API inventory static audit passed; installed catalog audit not run'
  exit 0
fi
...
for _ in {1..120}; do
  if docker compose exec -T postgres psql -XAtq -U postgres -d postgres -c \
      "SELECT extversion = '0.46.0' FROM pg_extension WHERE extname = 'pg_react'" 2>/dev/null | grep -qx t; then
    ready=1
    break
  fi
  sleep 1
...
echo "v0.46.0 installed API catalog audit passed: $artifact"
