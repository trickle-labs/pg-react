...
expected_version=${M54_EXPECTED_VERSION:-0.43.0}
...
static_audit() {
  for file in \
    ...
    docs/versioning.md sql/m54.sql tests/m54.sql; do
    test -s "$file"
  done
  bash tests/current-docs.sh
  bash tests/api-inventory.sh
  bash tests/workflow-syntax.sh
  bash -n tests/m54.sh tests/current-docs.sh tests/api-inventory.sh tests/workflow-syntax.sh
  jq -e --arg version "$expected_version" '.extension_version == $version and .contract_version == 54' docs/m54-api-inventory.json >/dev/null
  cmp sql/pg_react--0.42.0--0.43.0.sql sql/m54.sql
  cmp sql/pg_react--0.43.0.sql <(awk 'FNR==NR{part[++n]=$0; next} {print} END{for (i=1; i<=n; i++) print part[i]}' sql/m54.sql sql/pg_react--0.42.0.sql)
...
if [[ $profile = complete ]]; then
  ...
  export PG_REACT_INIT_VERSION=0.42.0
  docker compose up -d --no-build >/dev/null 2>&1
  wait_for_version 0.42.0
  ...
  run_test 'M54 adjacent upgrade' docker compose exec -T postgres psql -XAtq -U postgres -d postgres -v ON_ERROR_STOP=1 -c "ALTER EXTENSION pg_react UPDATE TO '0.43.0';"
  run_test 'M54 upgraded version' docker compose exec -T postgres psql -XAtq -U postgres -d postgres -v ON_ERROR_STOP=1 -c "SELECT extversion = '0.43.0' FROM pg_extension WHERE extname = 'pg_react';"
  ...
  docker compose up -d --no-build >/dev/null 2>&1
  wait_for_version 0.42.0
  run_test 'M54 rollback version' docker compose exec -T postgres psql -XAtq -U postgres -d postgres -v ON_ERROR_STOP=1 -c "SELECT extversion = '0.42.0' FROM pg_extension WHERE extname = 'pg_react';"
