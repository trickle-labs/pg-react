...
BEGIN
    ...
    IF (SELECT jsonb_agg(to_jsonb(evidence) ORDER BY rule_name, public_group_key)
       ...
       OR pgreact_api.explain('m16_recovery.alert', '2'::jsonb)
          IS DISTINCT FROM expected.false_explanation
       OR pg_is_in_recovery() THEN
        RAISE EXCEPTION 'M16 promoted physical restore changed typed aggregate state';
    END IF;
END
$$;
SELECT 'M16 standby promotion and physical restore preserved typed aggregates';
