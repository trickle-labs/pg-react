...
One factual clarification: **M33 is the v1 qualification milestone, but its proposed release is `v0.29.0`**. The documented sequence is M30 (applicability foundation), M31 → `0.27.0`, M32 → `0.28.0`, M33 → `0.29.0`, followed by `1.0.0` only after the M33 evidence and release gates pass.
...
* **Getting there in exactly M30–M33, without an additional stabilization release or material de-scoping:** roughly **40%**
* **Reaching v1 after M31 has passed its full intended gates:** roughly **80% or better**

There is no calendar schedule in the roadmap, so I cannot attach that probability to a date. The real question is less “will v1 happen?” and more “will M30 and M31 remain bounded and reviewable?”

## What the roadmap gets right

The sequencing is excellent:

1. **M30: establish the applicability foundation**
2. **M31: establish authoritative runtime truth**
3. **M32: make the correct system usable**
4. **M33: freeze, harden, and prove it**
...
## M30 and M31 are the make-or-break milestones

M30 and M31 are described as convergence and correctness work, but their proposal reveals something closer to a **cross-cutting runtime rearchitecture**.
...
## M32 is more than polish

M32 is called an interface milestone, but it includes a meaningful public-interface consolidation:
...
That is good work, but it can uncover conceptual problems that feed back into M31. Real users frequently expose that apparently sound abstractions require too much hidden state, too many identifiers, or too much knowledge of internal object categories. M32 therefore needs to begin as a design-validation stream during M31, even if its implementation ships afterward.

## M33 is comparatively well bounded

M33 itself is not what worries me. Its discipline is one of the strongest parts of the plan:
...
If M30 establishes the foundation, M31 genuinely eliminates façade/backend divergence, and M32 validates the public API with independent users, M33 should be a manageable qualification exercise.

The danger is allowing M33 to become the first time the full upgrade, recovery, concurrency, and security matrices are exercised. In that case, M33 will discover architectural defects rather than merely prove readiness.

## Execution risk

As of **August 16, 2026**, `v0.26.0` is the baseline. The only visible commit on `main` after that release is the documentation commit defining the revised M30–M33 program; no M30 implementation has yet landed on `main`.
...
Rapid delivery through earlier milestones is encouraging, but it should not be extrapolated linearly. M30 and M31 are qualitatively different from documentation, packaging, or additive API work.
...
* **M30 — Applicability foundation:** identity model, scope semantics, disposition matrix, relational eligibility/support schema, migrations, inspection primitives, and the failing/then-passing applicability fixtures.
* **M31 — Authoritative runtime:** adapters, global coordinator, lock ordering, lifecycle/support transitions, claimed-work revalidation, frontier/barrier semantics, race/recovery testing, and release qualification.

No later gate should proceed on an assumed or façade-level version of the earlier one.

### 3. Run M33 evidence continuously

Fresh installs, direct upgrades from `0.26.0`, rollback/recovery tests, role isolation, packaged-artifact testing, and basic performance budgets should run throughout M30, M31, and M32. M33 should consolidate existing green evidence, not begin collecting it.

### 4. Add independent review before M31 closes
...
The five M32 usability participants should also be recruited before M32 starts so their feedback can influence the API before it freezes.

### 5. Plan for a real release candidate

Even after `0.29.0`, I would expect at least one `1.0.0-rc` cycle using the exact packaged artifact, including both fresh installations and real upgrades from the supported pre-v1 baseline. That is consistent with the roadmap’s standards and should not be treated as a failure to “finish by M33.”
...
But I would not yet predict that it will happen cleanly in exactly four more milestones. **M30 and M31 contain most of the remaining technical uncertainty and probably 60–70% of the delivery risk.** M32 and M33 are credible only if M30's support matrix is narrowed where necessary and M31's acceptance is based on independently reviewed, end-to-end runtime truth—not merely passing façade tests.

My go/no-go judgment would therefore be:

> **Proceed with the roadmap, but treat M30 and M31 as two hard milestones. Do not promise the v1 cut until M31 demonstrates zero façade/backend divergence across every object kind claimed as supported.**
