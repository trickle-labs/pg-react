...
BEGIN
    ...
    END IF;
    digest_kind := CASE WHEN (preview_result -> 'summary') ? 'plan_digest'
...
CREATE OR REPLACE FUNCTION pgreact_internal.replace_pack_rule(
...
DECLARE
    ...
    next_version uuid;
    orphan_rule uuid;
    temporary_name text := rule_name || '#m54-' || replace(gen_random_uuid()::text, '-', '');
BEGIN
    ...
    PERFORM pgreact_internal.maybe_fail_pack('generated');
...
DO $m54$
BEGIN
    IF to_regprocedure('pgreact.remove(text,jsonb)') IS NOT NULL
       AND to_regprocedure('pgreact.remove_m53(text,jsonb)') IS NULL THEN
        ALTER FUNCTION pgreact.remove(text, jsonb) RENAME TO remove_m53;
    END IF;
END
$m54$;
...
AS $m54$
BEGIN
    PERFORM pg_advisory_xact_lock(hashtextextended('rule:' || name, 5788046901200000));
    PERFORM pg_advisory_xact_lock(hashtextextended('decision_program:' || name, 5788046901200000));
    PERFORM pg_advisory_xact_lock(hashtextextended('policy_set:' || name, 5788046901200000));
    RETURN pgreact.remove_m53(name, preconditions);
END
$m54$;

DO $m54$
BEGIN
    IF to_regprocedure('pgreact.validate(pgreact_api.declaration)') IS NOT NULL
       AND to_regprocedure('pgreact.validate_m53(pgreact_api.declaration)') IS NULL THEN
        ALTER FUNCTION pgreact.validate(pgreact_api.declaration) RENAME TO validate_m53;
    END IF;
    IF to_regprocedure('pgreact.preview(pgreact_api.declaration,jsonb)') IS NOT NULL
       AND to_regprocedure('pgreact.preview_m53(pgreact_api.declaration,jsonb)') IS NULL THEN
        ALTER FUNCTION pgreact.preview(pgreact_api.declaration, jsonb) RENAME TO preview_m53;
    END IF;
    IF to_regprocedure('pgreact.deploy(pgreact_api.declaration,jsonb)') IS NOT NULL
       AND to_regprocedure('pgreact.deploy_m53(pgreact_api.declaration,jsonb)') IS NULL THEN
...
CREATE OR REPLACE FUNCTION pgreact_api.deploy(
    declaration pgreact_api.declaration, review_token text,
    preconditions jsonb DEFAULT '{}'::jsonb
)
RETURNS jsonb
