...
COPY src/ src/
RUN cargo build --locked --release --jobs 1 --no-default-features --features pg18
...
ENV PG_REACT_INIT_VERSION=0.11.0

COPY pg_react.control /usr/share/postgresql/18/extension/pg_react.control
COPY sql/ /usr/share/postgresql/18/extension/
COPY --from=builder /build/target/release/libpg_react.so /usr/lib/postgresql/18/lib/pg_react.so
