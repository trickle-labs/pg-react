...
M10 admits one stratified aggregate edge without allowing aggregation inside a recursive component. A keyed rule may compare grouped `COUNT(*)` over a completed lower stratum with an immutable `bigint` threshold. Its durable evidence records the group key, exact count, comparison, and lower frontier; changes that do not flip the comparison update that evidence without creating a false lifecycle transition. Positive recursion may remain within a stratum, but every aggregate dependency points strictly downward. Richer aggregates and aggregate cycles remain separate work.
