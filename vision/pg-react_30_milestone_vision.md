...
> Planning status (August 2026): M38 is complete, extension `0.35.0` is the current qualified baseline, and M39 is the current milestone. `1.0.0` and its complete feature freeze are postponed indefinitely. [`ROADMAP.md`](../ROADMAP.md) remains the canonical milestone schedule.
...
This vision begins with M35 and describes 30 possible milestones through M64. M35 through M38 are complete, and M39 is the current milestone in `ROADMAP.md`. M40 through M64 are possibilities, not a queue and not a promise that every item will ship. Their numbers give the team stable names for discussion. Evidence may change the order, combine milestones, replace them, or show that one should never be built.

## Decision horizons

### Committed horizon

The project commits to one milestone at a time. M39 is the current milestone. Later work enters the canonical roadmap only when it has acceptance criteria that tests can execute, a named owner, evidence that it helps the initial market, and evidence from the preceding milestone. This keeps a long vision from turning into a long list of promises.
