...
> Current state: M44 and extension `0.41.0` are pg-react's qualified baseline. M53 complete policy-set packaging is the current planning milestone. The repository still contains the prepared `1.0.0-rc.1` candidate, but the `1.0.0` release and its complete feature freeze are postponed indefinitely. The qualified stack uses PostgreSQL 18.3 and pg_trickle 0.81.0 on Linux `amd64`. See the [support matrix](../docs/v1-support-matrix.md) for the full boundary. This document explains how the projects work together. Each project's documentation remains the source for its current releases and APIs.
