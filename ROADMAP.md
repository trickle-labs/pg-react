...
- one continuously true semantic match creates one activation generation;
...
- `pgreact.create_rule`, pause, resume, replace, and inspect operations.
...
- `pgreact.explain_rule` and `pgreact.health_check` with practical remediation hints.
...
- Registration rejects non-view definitions, null or duplicate semantic keys, incompatible typed signatures, unsafe roles, and unsupported query capabilities before activation.
...
- Constraint and activate-only command rules can be installed, explained, paused, resumed, replaced, and removed cleanly.
...
- Generic transactional outbox with deterministic idempotency keys and a stable event envelope.
- Execution-attempt history and operator APIs for retry, cancel, reconcile, and lease repair.
...
- Outbox insertion and episode completion are atomic.
...
- Reconciliation is idempotent and respects its configured event-emission policy.
...
- Retention, cleanup, vacuum, and catalog-growth controls.
- Metrics, structured logs, health endpoints, and alerts for drift, invalid bindings, reconciliation, agenda depth, latency, failures, lease expiry, and outbox state.
- Reproducible packages or images for every supported platform.
- Benchmark suite covering no-change refreshes, activation bursts, high change-event volume, many rules, reconciliation, claims, and consequence execution.
- At least one real pilot use case operated through failure and recovery exercises.
...
| **External effects** | At-least-once semantics and deterministic idempotency are documented and exercised; no exactly-once claim is implied |
| **Usability** | Reference authoring and recovery tasks succeed through public APIs with actionable diagnostics |
| **Operations** | Health, metrics, retention, repair, and runbooks cover each supported failure mode |
| **Performance** | Benchmarks have published baselines and regression budgets appropriate to the supported scale |
...
- The default constraint bootstrap policy and the explicit command bootstrap choices.
...
- The stable outbox envelope and idempotency contract.
...
- Retention defaults and upgrade compatibility windows.
