...
| pg-react artifact | `v0.36.0` release image and extension files; documented adjacent update `0.35.0 -> 0.36.0` | Releases before `0.31.0`; unreleased or untracked candidate versions |
...
| Simulation | Current or typed hypothetical comparison; caller-supplied replay; at most two backtest sides; bounded why-changed evidence | History capture or reconstruction; durable simulation jobs; more than two sides; general why-not answers |
...
The `v0.36.0` extension is built with pgrx 0.18.0 and Rust 1.89.0. These identify
...
- The managed runtime accepts extension versions `0.31.0` through `0.36.0`,
