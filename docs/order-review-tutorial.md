...
Stop if the extension versions or worker protocol differ. The supported server is PostgreSQL 18.4 with pg-react `0.45.0` and pg_trickle 0.98.0. Installation and preload details live in [Installation](installation.md), while runtime diagnosis belongs in [Operations](operations.md).
